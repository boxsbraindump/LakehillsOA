...
import { ClipboardCheck, FileSearch, CreditCard, Sparkles } from "lucide-react";
...
export default function Sidebar() {
  return (
    <aside className="flex shrink-0 flex-col border-b border-(--color-hairline) bg-(--color-canvas) px-3 py-3 md:h-svh md:w-64 md:border-r md:border-b-0 md:py-4">
      <NavLink
        to="/"
        className="mb-2 flex items-center gap-2 rounded-(--radius-md) px-2 py-1.5 text-[15px] font-semibold text-(--color-ink) hover:bg-(--color-canvas-soft) md:mb-6"
      >
      ...
      <div className="mt-auto hidden px-2 pt-4 text-[12px] text-(--color-ink-faint) md:block">
